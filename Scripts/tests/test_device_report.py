...
""",
    ...
    def test_measurement_scripts_require_exact_model_and_id_destination(self) -> None:
        ...
        for script in (benchmark, memory):
            ...
            self.assertIn('1)\n    xcrun simctl shutdown "$DEVICE_ID"', script)
            self.assertIn('xcrun simctl erase "$DEVICE_ID"', script)
            self.assertNotIn('simctl bootstatus "$DEVICE_ID" -b', script)
            self.assertIn("xcodebuild owns booting the selected destination", script)
